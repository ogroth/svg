...
# HACK: epoch size for some datasets
...
def make_gifs(x, idx, name):
    ...
    for i in range(opt.batch_size):
        ...
        for t in range(opt.n_eval):
            ...
            gifs[t].append(posterior_gen[t][i])
            ...
            gifs[t].append(all_gen[sidx][t][i])
        ...
        gt_images, posterior_images, pred_images = [], [], []
        for t in range(opt.n_eval):
            gt_images.append(gifs[t][0])
            posterior_images.append(gifs[t][1])
            pred_images.append(gifs[t][2])
        utils.save_gif_v2('%s/%s_%03d.gif' % (opt.log_dir, 'gt', idx+i), gt_images)
        utils.save_gif_v2('%s/%s_%03d.gif' % (opt.log_dir, 'q', idx+i), posterior_images)
        utils.save_gif_v2('%s/%s_%03d.gif' % (opt.log_dir, 'pred', idx+i), pred_images)
